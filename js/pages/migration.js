...
const MigrationPage = {
                    ...
                    <div class="space-y-6">
                        ${[
                            ...
                            <div class="relative pl-10">
                                <div class="absolute left-2 top-1 w-5 h-5 rounded-full bg-white border-2 ${item.status === 'active' ? 'border-primary-500' : 'border-slate-300'} flex items-center justify-center">
                                    <span class="w-2 h-2 rounded-full ${item.status === 'active' ? 'bg-primary-500' : 'bg-slate-300'}"></span>
                                </div>
                                <div class="py-1">
    ...
    initRhythmChart() {
        ...
        const data = MockData.migrationRhythm;
    ...
    renderRoutes(container) {
        const selectedRoute = MockData.migrationRoutes[0];
            ...
            <div class="mb-6 flex items-center justify-between">
                ...
                <div class="flex items-center gap-3">
                    <select id="routeSelect" class="px-4 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-medium focus:outline-none focus:ring-2 focus:ring-primary-500">
                        ${MockData.migrationRoutes.map(route => `
                            <option value="${route.id}">${route.speciesName} - ${route.season}</option>
                        `).join('')}
                    </select>
                    ...
                    <div class="bg-white rounded-2xl p-5 shadow-sm border border-slate-100">
                        ...
                        <div class="space-y-3">
                            ${selectedRoute.stops.map((stop, index) => `
                                <div class="relative pl-6 pb-3 ${index < selectedRoute.stops.length - 1 ? 'border-l-2 border-primary-200' : ''} ml-2">
                                    <div class="absolute -left-[9px top-0 w-4 h-4 rounded-full bg-gradient-to-br from-primary-400 to-primary-600 border-2 border-white shadow-sm"></div>
                                    <div>
                                        <p class="text-sm font-medium text-slate-700">${stop.site}</p>
                                        <p class="text-xs text-slate-500 mt-0.5">
                                            ${stop.date} ${stop.stayDays > 0 ? `· 停留${stop.stayDays}天` : ''}
                                        </p>
                                    </div>
                                </div>
                            `).join('')}
    ...
    initMigrationMap() {
        ...
        const route = MockData.migrationRoutes[0];
        
        const map = L.map('migrationMap').setView([40, 118], 5);
        ...
        route.stops.forEach((stop, index) => {
            ...
            const icon = L.divIcon({
                ...
                html: `<div style="position: relative;">
                    <div style="width: ${isFirst || isLast ? 28 : 20px; height: ${isFirst || isLast ? 28 : 20}px; background: ${isFirst ? '#10b981' : isLast ? '#f43f5e' : '#0d9488'}; border-radius: 50%; border: 3px solid white; box-shadow: 0 2px 8px rgba(0,0,0,0.3); display: flex; align-items: center; justify-content: center; font-size: 10px; color: white; font-weight: bold;">
                        ${isFirst ? '<i class="fas fa-play" style="font-size: 10px;"></i>' : isLast ? '<i class="fas fa-flag-checkered" style="font-size: 10px;"></i>' : index + 1}
                    </div>
